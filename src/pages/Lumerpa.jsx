...
const Lumerpa = () => {
  ...
  // Kategori sidebar (sesuaikan penamaanmu; “Rafless” tetap mengikuti input)
  const categories = [
    "Alice", "Arias", "Asian", "Oslo", "Presentation", "Rafless", "Rim", "Sides", "Square", "Stackable"
  ];

  // --- Produk DINAMIS dari localStorage untuk SECTION + kategori terpilih
  const dynamicProducts = useMemo(
    () => getProducts(SECTION, selectedCategory),
    [selectedCategory]
  );

  // Gabungkan statis + dinamis
        ...
        <div className="w-3/4">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
            {filteredProducts.map((product) => (
              <div
                key={product.id}
                className="bg-white rounded-2xl overflow-hidden shadow-sm flex flex-col transition-transform duration-200 hover:-translate-y-1 hover:shadow-md"                onClick={() => {
                  // aman: ke detail Arias jika produknya kategori/nama Arias
                  if (
                    product.category === "Arias" ||
                    /arias/i.test(product.name || "")
                  ) {
