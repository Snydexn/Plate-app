...
const Arturo = () => {
  const [selectedCategory, setSelectedCategory] = useState("Arturo");
  ...
  const filtered = selectedCategory && selectedCategory !== "Semua legacy"
    ? products
    : products;
          ...
          <div className="space-y-2">
            {categories.map((category, idx) => (
              <button
                key={idx}
                onClick={() => {
                  if (category === "Semua Legacy") {
                    navigate("/legacy");
                    setSelectedCategory(null);
                  } else if (category === "Arturo") {
